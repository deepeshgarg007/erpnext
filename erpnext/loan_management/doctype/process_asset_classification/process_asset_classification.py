...
class ProcessAssetClassification(Document):
	def on_submit(self):
		...
		update_days_past_due_in_loans(
			posting_date=self.posting_date, loan_type=self.loan_type, loan_name=self.loan
		)


def create_process_asset_classification(posting_date=None, loan_type=None, loan=None):
